...
description: agent向けテキスト指示（skill / slash command / taskプロンプト / CLAUDE.md節 / コード生成プロンプト）を、バイアスを排した実行者（新規subagent）に動かしてもらい、両面（実行者の自己申告 + 指示側メトリクス）で評価して反復改善する手法。改善が頭打ちになるまで回す。skill/プロンプトを新規作成・大幅改訂した直後、claudemd-generatorでCLAUDE.mdを大幅更新した直後、またはエージェントの挙動が期待通りにならない原因を指示側の曖昧さに求めたいときにトリガーする。「skill評価」「プロンプト改善」「iterationで回して」「skillテスト」「プロンプトチューニング」「empirical」でも起動。コード設計のレビューはzero-base-review、PR静的ガードはharness cc-update-reviewと棲み分ける。
---
...
- `claudemd-generator` で CLAUDE.md / AGENTS.md を大幅更新した直後の動作検証
...
- 対象が「実装コードの設計品質」のとき → `zero-base-review` を使う
- 対象が「CC アプデ統合 PR の静的ガード」のとき → `claude-code-harness:cc-update-review` を使う
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。Agent tool で **新規 subagent を dispatch** する（`subagent_type: general-purpose` または `Explore`）。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。dispatch 不能環境の扱いは「環境制約」節を参照。
...
本スキルの立ち位置（この環境での棲み分け）:

| 対象 | 使うスキル |
|---|---|
| プロンプト / skill 本文の品質 | **empirical-prompt-tuning**（本スキル） |
| 実装コードの設計品質 | `zero-base-review` |
| CLAUDE.md / AGENTS.md の新規作成・更新 | `claudemd-generator`（更新後の検証に本スキルを連結すると効く） |
| CC アプデ統合 PR の静的ガード（書いただけ検出） | `claude-code-harness:cc-update-review` |
| 新規 skill の骨格作成 | `example-skills:skill-creator`（作成後のチューニングに本スキルを連結） |
| Web アプリ品質の探索的テスト | `dogfood`（対象がプロンプトではなく実アプリ）|

併用の典型フロー:

- `example-skills:skill-creator` で skill 作成 → 本スキルで 2〜3 iter チューニング → 収束で完了
- `claudemd-generator` で CLAUDE.md 更新 → 本スキルで「よくある依頼」シナリオ 2 本を subagent に投げて導線確認
- harness ワークフロー（`harness-plan` / `harness-work`）で実装中に挙動がぶれる → 指示側の曖昧さを疑う時だけ本スキルを発動（実装そのものは harness に任せる）

出典: mizchi/chezmoi-dotfiles の `empirical-prompt-tuning` を本環境向けに関連節と description のみ調整したもの。
